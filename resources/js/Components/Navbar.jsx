  ...
  return (
    <nav className={`w-full fixed top-0 z-50 font-poppins duration-300 ease-in-out text-primary300 bg-transparent ${isScrolled ? 'bg-opacity-50 backdrop-blur-sm' : ''}`}>
      <main className='flex container mx-auto justify-between items-center py-2'>
        ...
        {/* Mobile Navigation */}
        <ul className={`lg:hidden fixed top-0 left-0 h-screen py-4 w-3/4 px-2 bg-primary500 text-white text-lg font-medium transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <img src={LogoImage} className='w-32 ' alt=""  />
          {navItems.map((item, index) => (
            <NavItem key={index} className="lg:py-4 px-4 py-2 border-b lg:text-base text-sm border-primary300">
              {item}
            </NavItem>
          ))}
